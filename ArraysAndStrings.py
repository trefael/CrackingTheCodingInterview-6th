...
def IsUnique(myString):
    #Create dictionary
    Alphabet = {'a': 0, 'c': 0, 'b': 0, 'e': 0, 'd': 0, 'g': 0, 'f': 0, 'i': 0, 'h': 0, 'k': 0, 'j': 0, 'm': 0, 'l': 0, 'o': 0, 'n': 0, 'q': 0, 'p': 0, 's': 0, 'r': 0, 'u': 0, 't': 0, 'w': 0, 'v': 0,
     'y': 0, 'x': 0, 'z': 0}
    
    #Case string is larger then total alpabet 
...
def IsPermutation(myString1, myString2):
    ...
    for i in len(myString1):
        if myString1[i]!=myString2[i]:
            return False

    for i in len(myString2):
